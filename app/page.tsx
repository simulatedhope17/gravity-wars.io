...
const GRAVITY_CONSTANT = 2
const MAX_VELOCITY = 3
const COMPASS_SIZE = 140
const ENERGY_DECAY_RATE = 0.12
const GRAVITY_PULL_COST = 4
const CHARGING_RADIUS = 300 // Increased for easier energy collection
const GRAVITY_PULL_RADIUS = 180
const GRAVITY_PULL_STRENGTH = 0.25
const ORBITAL_ABSORPTION_RATE = 0.002 // Very slow absorption when orbiting
...
export default function GravityWarsGame() {
  ...
  const initGame = () => {
    ...
    const newPlayer: GameObject = {
      ...
      vy: 0,
      mass: 10,
      radius: calculateRadius(10, "player"),
      type: "player",
      color: "#00ffff",
      energy: 100,
      maxEnergy: 100,
      ...
      for (let j = i + 1; j < objects.length; j++) {
        ...
        if (distance < obj1.radius + obj2.radius) {
          ...
          if (!smaller.isGlowing && !larger.isGlowing) {
            larger.mass += smaller.mass * 0.7
            larger.radius = calculateRadius(larger.mass, larger.type)

            if (larger.energy && smaller.energy) {
              larger.energy = Math.min(larger.maxEnergy!, larger.energy + smaller.energy * 0.4)
            }

            if (larger.id === "player") {
              scoreIncrease += Math.floor(smaller.mass * 15)
            }
  ...
  return (
    <div className="w-full h-screen bg-black flex flex-col items-center justify-center relative overflow-hidden">
      {gameState === "menu" && (
        <Card className="p-8 text-center bg-gray-900 border-cyan-500">
          <h1 className="text-4xl font-bold text-cyan-400 mb-4">GRAVITY WARS.IO</h1>
          <p className="text-gray-300 mb-6">
            Explore the infinite cosmos! Orbit glowing bodies to charge energy safely.
            <br />
            Keep moving to avoid absorption. Discover space activities and mysteries!
          </p>
          <Button onClick={initGame} className="bg-cyan-600 hover:bg-cyan-700">
            ENTER THE COSMOS
          </Button>
        </Card>
      )}
      ...
      {gameState === "gameOver" && (
        <Card className="p-8 text-center bg-gray-900 border-red-500">
          <h2 className="text-3xl font-bold text-red-400 mb-4">COSMIC DEMISE</h2>
          <p className="text-gray-300 mb-4">Final Score: {score}</p>
          <p className="text-gray-400 mb-4">Activities Discovered: {discoveredActivities.length}</p>
          <Button onClick={initGame} className="bg-red-600 hover:bg-red-700">
            RESPAWN
          </Button>
        </Card>
      )}
      ...
      {gameState === "playing" && (
        <>
          ...
          <div className="absolute top-4 left-4 text-white">
            <div className="bg-black bg-opacity-95 p-4 rounded-lg border border-cyan-500">
              <div className="text-cyan-400 font-bold">COSMIC STATUS</div>
              <div>Score: {score}</div>
              <div>Mass: {displayStats.mass.toFixed(1)} units</div>
              <div>Radius: {playerRef.current?.radius.toFixed(1)} units</div>
              <div>Discoveries: {discoveredActivities.length}</div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm">{displayStats.energy.toFixed(0)}</span>
              </div>
              {playerRef.current?.absorptionProgress && playerRef.current.absorptionProgress > 0 && (
                <div className="text-red-400 text-sm mt-1">⚠️ Absorption Risk: Keep Moving!</div>
              )}
            </div>
          </div>
          ...
          <div className="absolute bottom-4 left-4 text-white text-sm">
            <div className="bg-black bg-opacity-95 p-3 rounded-lg border border-cyan-500">
              ...
              <div className="text-yellow-400">⚡ Larger charging radius (300)</div>
          ...
          {spaceActivitiesRef.current.filter((a) => !a.discovered).length > 0 && (
            <div className="absolute top-4 left-1/2 transform -translate-x-1/2 text-white">
              <div className="bg-black bg-opacity-95 p-3 rounded-lg border border-purple-500">
                <div className="text-purple-400 font-bold">NEARBY ACTIVITIES</div>
                {spaceActivitiesRef.current
                  .filter((a) => !a.discovered)
                  .slice(0, 3)
                  .map((activity) => (
                    <div key={activity.id} className="text-sm">
                      {activity.type === "derelictShip" && "🚀 Derelict Ship"}
                      {activity.type === "ancientRelic" && "⚱️ Ancient Relic"}
                      {activity.type === "energyCrystal" && "💎 Energy Crystal"}
                      {activity.type === "spaceWhale" && "🐋 Space Whale"}
                      {activity.type === "alienStructure" && "🛸 Alien Structure"}
                    </div>
                  ))}
              </div>
            </div>
          ...
          {cosmicEventsRef.current.length > 0 && (
            <div className="absolute top-4 right-4 text-white mr-44">
              <div className="bg-black bg-opacity-95 p-3 rounded-lg border border-orange-500">
                <div className="text-orange-400 font-bold">COSMIC EVENTS</div>
                {cosmicEventsRef.current.map((event) => (
                  <div key={event.id} className="text-sm">
                    {event.type === "solarFlare" && "🌟 Solar Flare"}
                    {event.type === "meteorShower" && "☄️ Meteor Shower"}
                    {event.type === "nebula" && "🌌 Nebula"}
                    {event.type === "asteroidBelt" && "🪨 Asteroid Belt"}
                    {event.type === "quantumAnomaly" && "⚛️ Quantum Anomaly"}
                    {event.type === "wormholeStorm" && "🌀 Wormhole Storm"}
                    {event.type === "solarWind" && "💨 Solar Wind"}
                    {event.type === "coronalMassEjection" && "🔥 Coronal Mass Ejection"}
                    {event.type === "magneticStorm" && "⚡ Magnetic Storm"}
                  </div>
                ))}
              </div>
            </div>
